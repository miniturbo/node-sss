            ...
            if (session && session.verified) {
                if (data.type === 'remote' && data.detail) {
                    ...
                    if (data.detail === 'sync') {
                        ...
                        client.broadcast({ type: 'remote', detail: 'sync', value: pageIndex[syncIndex] });
                    }
                    // clientにブロードキャストする
                    else {
                        if (syncIndex && pageIndex[syncIndex]) {
                            var num = data.detail === 'next' ? pageIndex[syncIndex] + 1 : pageIndex[syncIndex] - 1;
                            client.broadcast({ type: 'remote', detail: 'sync', value: num });
                        }
                        else {
